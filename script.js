...
function calculate() {
  ...
  const extras =
    ...
    drinks * 0.12;
  const totalLiters = Math.min(Math.max(baseLiters + activityLiters + extras, 1.5), 6);
  ...
  elements.activity.textContent = `${rounded(activityLiters, 1).toFixed(1)} L`;
  elements.extra.textContent = `${rounded(extras, 1).toFixed(1)} L`;
}
